...
workspace-specific. Check your available MCP servers for the correct name.
See `phases-rules/04-reproduce.md` for project-specific MCP and screenshot
conventions.
...
```
CallMcpTool: <your-playwright-mcp-server>
  browser_navigate -> { "url": "<consoleUrl>" }
```
...
**Read project-specific screenshot rules from `phases-rules/04-reproduce.md` before
taking any screenshots.** If no phases-rules file exists, save screenshots to
`~/Downloads/${TICKET_KEY}/` with descriptive names.
...
```
CallMcpTool: browser_network_requests -> { "static": false, "requestBody": false, "requestHeaders": false }
```
